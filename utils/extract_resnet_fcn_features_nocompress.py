...
def main(args):
    ...
    print('Time till tf session is created =', str(cur_time - code_start_time))
    with tf.Session() as sess:
        ...
        cur_time = datetime.datetime.now()
        print('Time from start till start of batches =', str(cur_time - code_start_time))
        ...
        for image_file in image_files:
            ...
            if num_images_in_batch >= args.batch_size or num_images_processed >= max_images_to_process:
                ...
                batch_input_time = datetime.datetime.now()
                print('Time to get batch input =', str(batch_input_time - batch_start_time))
                
                output, activations = sess.run([net, all_layers], feed_dict=feed_dict)
                net_done_time = datetime.datetime.now()
                print('Tiem to run net =', str(net_done_time - batch_input_time))
                ...
                write_done_time = datetime.datetime.now()
                print('Time to assign to h5py dataset =', str(write_done_time - net_done_time))
